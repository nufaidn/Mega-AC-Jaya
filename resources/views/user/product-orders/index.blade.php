    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6 p-4 md:p-6 bg-gray-50 min-h-screen">
            ...
            <div class="relative z-10">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="text-white/90 text-lg">Kelola dan pantau semua pesanan produk AC Anda</p>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="text-sm text-white/80">Total Orders</span>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-lg transition-all duration-300 hover:shadow-xl">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-xl md:text-2xl font-bold text-gray-900">Daftar Pesanan Anda</h3>
                        </div>
                        <p class="text-sm text-gray-500 mt-1 ml-12">Semua pesanan produk AC dalam satu tempat</p>
                    </div>
                    <div class="flex flex-wrap gap-3">
                        ...
                        <a href="{{ route('product-orders.create') }}" class="px-4 py-2.5 bg-gradient-to-r from-wa-600 to-wa-700 text-white rounded-xl hover:from-wa-700 hover:to-wa-800 transition-all duration-300 hover:shadow-lg flex items-center gap-2 font-medium">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-5 md:p-6">
                ...
                @php
                    $hasPendingPayments = $productOrders->where('payment_status', 'pending')->count() > 0;
                @endphp
                ...
                <div class="mb-6 p-4 bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-xl shadow-sm">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            <h4 class="font-semibold text-yellow-800 mb-1">Ada Product Yang Belum Di Bayar</h4>
                            <p class="text-sm text-yellow-700">Setelah melakukan pembayaran di Xendit, klik tombol <strong>"Cek Status"</strong> untuk memperbarui status pesanan Anda.</p>
                        </div>
                    </div>
                </div>
                @endif
                ...
                <div class="text-center py-12 md:py-16">
                    ...
                    <p class="text-gray-600 mb-8 max-w-md mx-auto text-lg">Mulai dengan membuat pesanan produk AC pertama Anda untuk melihatnya di sini.</p>
                    <a href="{{ route('product-orders.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-wa-600 to-wa-700 text-white rounded-xl hover:from-wa-700 hover:to-wa-800 transition-all duration-300 hover:shadow-lg font-medium text-lg">
                    ...
                    <div class="group relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-5 md:p-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div class="flex items-center gap-4">
                                ...
                                <img src="{{ asset('images/' . $order->product->image) }}" alt="{{ $order->product->name }}" class="w-16 h-16 rounded-xl object-cover flex-shrink-0">
                                @else
                                <div class="w-16 h-16 rounded-xl bg-gradient-to-br from-wa-500/10 to-wa-600/10 flex items-center justify-center flex-shrink-0">
                                ...
                                @endif
                                <div>
                                    <h4 class="text-lg font-bold text-gray-900 group-hover:text-wa-600 transition-colors">{{ $order->product->name }}</h4>
                                    <div class="flex items-center gap-2 text-sm text-gray-600 mt-1">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                                        </svg>
                                        <span>Qty: {{ $order->quantity }} • {{ $order->created_at->format('M d, Y') }}</span>
                                    </div>
                                    <div class="flex items-center gap-2 text-sm text-gray-600 mt-1">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                        </svg>
                                        <span>{{ $order->full_name }} • {{ $order->phone_number }}</span>
                                    </div>
                            ...
                            <div class="flex flex-col items-end gap-3 mt-4 md:mt-0">
                                <div class="flex flex-wrap gap-2 justify-end">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ 
                                        $order->status === 'completed' ? 'bg-gradient-to-r from-green-100 to-green-200 text-green-800 border border-green-300' : 
                                    ...
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $order->payment_status === 'paid' ? 'bg-gradient-to-r from-wa-100 to-wa-200 text-wa-800 border border-wa-300' : ($order->payment_status === 'pending' ? 'bg-gradient-to-r from-orange-100 to-orange-200 text-orange-800 border border-orange-300' : 'bg-gradient-to-r from-gray-100 to-gray-200 text-gray-800 border border-gray-300') }}">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <a href="{{ $order->payment_url }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-red-500 to-red-600 text-white text-sm rounded-xl hover:from-red-600 hover:to-red-700 transition-all duration-300 hover:shadow-lg font-medium">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
